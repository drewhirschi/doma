...
export function SectionView({ section, report }: ISectionViewProps) {
  ...
  const [drafterOpened, { open: openDrafter, close: closeDrafter }] =
    useDisclosure(false);
    ...
    validate: {
      exclusions: (value) => {
        ...
        const invalidDomains = domains.filter(
          (d) =>
            !/^[a-zA-Z0-9][a-zA-Z0-9-]{1,61}[a-zA-Z0-9]\.[a-zA-Z]{2,}$/.test(d),
        );
    ...
    >
      ...
      <Group justify="flex-end" gap={"xs"}>
        <Indicator
          ...
          // processing
        >
          <Button variant="default" size="compact-sm" onClick={openDrafter}>
            Search
          </Button>
        </Indicator>
      ...
      >
        ...
        <Fieldset legend="Search parameters">
          ...
          <TextInput
            label="Exclusions"
            description="Websites we should exclude (comma separated)"
          ...
          <Button
            onClick={async () => {
              const searchResults = await getSearchResults(
                [`${report.topic}: ${section.title}`],
                5,
                searchAndDraftform.values.recency,
              );
              ...
              const summaries = await Promise.all(
                searchResults.map(async (result) => {
                  const options = {
                    user:
                      `Summarize the following article about "${section.title}" in ${report.topic}` +
                      `\n<arcticle>\n${result.text}\n</arcticle>`,
                  };
              ...
              const resultsForInsert: Omit<SearchResult_SB, "id">[] =
