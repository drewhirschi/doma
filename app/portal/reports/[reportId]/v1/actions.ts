...
export async function searchAllSections(
  ...
  const queryProms = sectionsData.map(async (section: ISection) => {
    const sectionQueries = await generateSearchQueries(
      `${topic}: ${section.name}`,
      QUERIES_PER_SECTION,
    );
  ...
  fs.writeFileSync(
    "sections_health_with_searchdata.json",
...
export async function writeDraft(topic: string, sections: ISection[]) {
  ...
  return JSON.parse(
    fs.readFileSync("sections_health_full.json", "utf-8"),
  ) as ISection[];
...
export async function writeSection(
  ...
  const completion = await getLLMResponse({
    user: prompt + referenceMaterialPrompt,
  });
  return completion;
}
