...
export default async function Page({ params }: { params: { id: string, contractId: string } }) {
    ...
    const [contractQ, parsletQ] = await Promise.all([
        supabase.from("contract").select("*, annotation(*), extracted_information(*, contract_line(*)), contract_note(content), extract_jobs(*), formatted_info(*)").eq("id", params.contractId).single(),
