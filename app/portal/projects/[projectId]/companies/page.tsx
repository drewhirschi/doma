...
import { ActionIcon, Anchor, Checkbox, Group, Image, Table, TableTbody, TableTd, TableTh, TableThead, TableTr, } from '@mantine/core';
...
export default function Page({ params }: { params: { projectId: string }}) {
    ...
    const { data: project, error, loading } = useSBFetch(() => supabase.from("ib_projects").select("*, model_cmp(*), company_profile!deal_comps(*) ").eq("id", params.projectId).single());
    ...
    const rows = project!.company_profile?.map((element) => (
        ...
        <TableTr key={element.id} onClick={() => toggleRow(element.id)} 
    ...
    return (
        ...
        <Group justify='end'>
            ...
            <AddCompaniesPanel  />
            <RemoveCompaniesDialog selectedCompanies={selectedRows} projectId={params.projectId} onRemoveSuccess={() => setSelectedRows([])} />
        </Group>
