...
import { companyIdSchema } from "../jobTypes.js";
...
export async function transactionDiscovery(job: SandboxedJob<z.infer<typeof companyIdSchema>>) {
    ...
    const searchProms = queriesRes.queries.map(async query => {
        ...
        return items.map(item => ({ query, full_query: searchQuery, query_emb: JSON.stringify(queryEmb), url: item.link, title: item.title }))
    })

    const insertItems = (await Promise.all(searchProms)).flat()


    const insertArticles = await sb.from('transaction_search_res').upsert(insertItems, { onConflict: 'url', ignoreDuplicates: true }).select()


    if (insertArticles.error) {
        console.error(insertArticles.error)
        const err = new Error(`Error inserting transaction search results for cmpId: ${job.data.cmpId}. Error was: ${insertArticles.error.message}`)
