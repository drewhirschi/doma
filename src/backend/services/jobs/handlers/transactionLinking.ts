...
export async function transactionCompanyLinking(job: Job<z.infer<typeof transactionLinkingSchema>>) {
    ...
    const transParticipantProms = [
        ...
        .map(async involvedParty => {
            ...
            const bestCandidateWebsite = await getStructuredCompletion({
                ...
                user: potentialSites.map((x, i) => JSON.stringify({ id: i,url: x.link, snippet: x.snippet, title: x.title})).join("\n"),
            })
    ...
    companyProfiles.map(async transactionParticipant => {
        ...
        let companyId = 0
        const companyCheck = await supabase.from("company_profile").select().eq("origin", transactionParticipant.websiteOrigin).maybeSingle()
        ...
        if (!companyCheck.data) {
            const insert = await supabase.from("company_profile").insert({ name: transactionParticipant.name, origin: transactionParticipant.websiteOrigin }).select()
            if (insert.error) {
                throw insert.error
            }
        ...
        const createRelation = await supabase.from("transaction_participant").insert({ trans_id: transactionParticipant.trans_id, cmp_id: companyId, role: transactionParticipant.role }).select()
        ...
        await industryQueue.add('scrape_company_website', { url: transactionParticipant.websiteOrigin });
