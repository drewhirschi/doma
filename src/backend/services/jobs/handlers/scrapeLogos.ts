...
async function scrapeImgLogos(sb: SupabaseClient, company: CompanyProfile_SB) {
    ...
    const dbLogosProms = completion?.possibleLogos.map(async logo => {
        ...
        try {
            ...
            return await uploadLogo(sb, fileName, company.id, buffer, logo.alt, response.headers['content-type'] ?? 'image/png')
...
async function uploadLogo(sb: SupabaseClient<Database>, fileName: string, cmpId: number, buffer: Buffer, alt: string, type: string) {
    ...
    if (error) {
        ...
        const { error: updateError, data: uploadedLogo } = await sb
            .from('cmp_logos')
            .upsert({ url: publicUrl, path: data.path, cmp_id: cmpId, alt }).select().single()
