...
export async function exchangeCodeForToken(
  ...
  let tokenError = null;
  try {
    tokenRes = await axios.post<MicrosoftTokenResponse>(
...
export async function refreshToken(sb: SupabaseClient<Database>) {
  const userId = (await sb.auth.getUser()).data.user?.id;
  const user = await sb.from("profile").select().eq("id", userId!).single();
  ...
  const data = {
    ...
    scope: EMAIL_SCOPES,
    refresh_token: (
      user.data.send_email_tokens as unknown as MicrosoftTokenResponse
    ).refresh_token,
...
export function authUrl(state?: string) {
  ...
  microsoftUrlParams.set(
    "redirect_uri",
    process.env.AUTH_URL + "/api/v1/auth/callback/microsoft",
  );
